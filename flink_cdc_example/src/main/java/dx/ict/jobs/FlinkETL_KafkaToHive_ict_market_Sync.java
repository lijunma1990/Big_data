 ...
 * @Version 1.0
 */
public class FlinkETL_KafkaToHive_ict_market_Sync {
    /**
     * flink cdc Mysql整库同步
     */
...
//        String mysqlSourceHeader = "mysql_source";
        ...
        String database = "ict_market";
        // 如果是整库，tableList = ".*"
        String tableList = "ict_market.market_data,ict_market.market_reserve_target";
        int port = 3306;
...
//        String sink_database = "test";
        String hive_sink_database = "ods_ict_market";
        ...
        String topics = "MysqlToKafka_Sync_ict_market";
        String topicPattern = "";//example:"^ods_icity_.*"
        String groupId = "Flink_ETL_Sync_ict_market";
...
//                final RowTypeInfo item = tableTypeInformationMap.get("demo1");
...
//            };
                ...
                final SingleOutputStreamOperator<Row> tmp_dataStream = kafkaSources.get(kafkaDataSourceName)
                        ...
                        .map(
                                new MapFunction<Tuple5<String, String, Row, String, String>, Tuple2<Row, String>>() {
                                    @Override
                                    public Tuple2<Row, String> map(Tuple5<String, String, Row, String, String> value) throws Exception {
...
//                                    System.out.println(value.f2);
                                        return Tuple2.of(rowOut, pt);
                                    }
                                }
                        )
//                        .assignTimestampsAndWatermarks(
//                                new BoundedOutOfOrdernessTimestampExtractor<Tuple2<Row, String>>(Time.seconds(10)) {
//                                    @Override
//                                    public long extractTimestamp(Tuple2<Row, String> element) {
...
//                        )
                        .assignTimestampsAndWatermarks(
                                WatermarkStrategy
                                        .<Tuple2<Row, String>>forBoundedOutOfOrderness(Duration.ofSeconds(10L))
                                        .withTimestampAssigner(
                                                new SerializableTimestampAssigner<Tuple2<Row, String>>() {
                                                    @Override
                                                    public long extractTimestamp(Tuple2<Row, String> element, long recordTimestamp) {
                                                        return Long.parseLong(element.f1);
                                                    }
                                                }
                                        )
                        ...
                        .process(
                                new ProcessFunction<Tuple2<Row, String>, Row>() {
                                    @Override
                                    public void processElement(Tuple2<Row, String> value, Context ctx, Collector<Row> out) throws Exception {
                                        String changeType = value.f0.getKind().toString();
                                        switch (changeType) {
                                            case "INSERT":
//                                                ctx.output(insertTag, value.f0);
                                                ctx.output(updateTag, value.f0);
                                                break;
                                            case "DELETE":
                                                ctx.output(deleteTag, value.f0);
                                                ctx.output(updateTag, value.f0);
                                                break;
                                            case "UPDATE_BEFORE":
                                                ctx.output(update_beforeTag, value.f0);
                                                break;
                                            case "UPDATE_AFTER":
                                                ctx.output(update_afterTag, value.f0);
                                                ctx.output(updateTag, value.f0);
                                                break;
                                            default:
                                                break;
